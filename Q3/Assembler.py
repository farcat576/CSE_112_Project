...
opcode = {"add": ("10000", 'A'), "sub": ("10001", "A"), "mov": ("1001", ""), "ld": ("10100", "D"),
          ...
          "addf": ("00000","A"), "subf": ("00001", "A"), "movf": ("00010","B")}
...
error_dict = {"101": "duplicate hlt statement detected.", "102": "Last instruction is not hlt.",
              ...
              "202": "Immediate value format not recognised",
              "203": "Illegal use of FLAGS register detected.",
              ...
              "402": "The sum/difference can not be represented using given notation"
              }
...
def type_A(reg1: str, reg2: str, reg3: str) -> str:
    return '00' + reg[reg1] + reg[reg2] + reg[reg3]


def type_B(reg1: str, imm: str) -> str:
    return reg[reg1] + bin(int(imm))[2:].rjust(8, '0')


def type_C(reg1: str, reg2: str) -> str:
    return '00000' + reg[reg1] + reg[reg2]


def type_D(reg1: str, memory: str) -> str:
    return reg[reg1] + var_dict[memory]


def type_E(memory: str) -> str:
...
def float_check(f: str) -> bool:
    ...
    if f[0] == '$' and whole.isdigit() and decimal.isdigit():
        return float(f[1:]) >= 0 and float(f[1:]) <= 252.0
    else:
        return False
...
def addf_subf_check(line: list[str], error_ln: str) -> bool:
    if not float_to_bin(bin_to_float(line[2]) + (bin_to_float(line[3]))):
        error("402",error_ln)
    else:
        return True
...
def movf_check(line: str, error_ln : str) -> bool:
    if len(line) !=3:
        ...
        error("401",str(int(error_ln)))
    return True

#immediate value is not checked, pls fix
def B_check(line: str, error_ln: str) -> bool:
    if len(line) != 3:
        error("204", error_ln)
    elif not reg_check(line[1]):
        if flags_check(line[1]):
...
def D_check(line: str, error_ln: str) -> bool:
    if len(line) != 3:
        error("204", error_ln)
...
def E_check(line: str, error_ln: str) -> bool:
    if len(line) != 2:
        error("204", error_ln)
...
def F_check(line: str, error_ln: str) -> bool:
    if line != ["hlt"]:
        error("204", error_ln)
    return True
...
def process():
    ...
    for num in op_dict:
        ...
        if op == "1001": #Defining exclusive case for mov to check whether it is type B or type C
            ...
            movf_check(line,str(int(num) + 1))
            L.append(op + reg(line[1]) + float_to_bin(line[2][1:]))
        else:
            if type == "A":
                if op == "00000" or "00001":
                    addf_subf_check(line,str(int(num) + 1)) 
                else:
